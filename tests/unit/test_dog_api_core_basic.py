...
def test_list_breeds(dog_api) -> None:
    ...
    :return: None
    """
    expected_response = [{'weight': {'imperial': '50 - 60', 'metric': '23 - 27'},
                          'height': {'imperial': '25 - 27', 'metric': '64 - 69'},
                          'id': 2, 'name': 'Afghan Hound', 'country_code': 'AG',
                          'bred_for': 'Coursing and hunting', 'breed_group': 'Hound',
                          'life_span': '10 - 13 years', 'temperament': 'Aloof, Clownish, Dignified, Independent, Happy',
    ...
    actual_response = dog_api.list_breeds(query_dict={"attach_breed": 1, "page": 1, "limit": 1})
...
def test_list_breeds_wrong_arg_type_value_error(dog_api) -> None:
    with pytest.raises(ValueError):
        actual_response = dog_api.list_breeds(query_dict="Invalid")


def test_create_vote_wrong_arg_type_value_error(dog_api) -> None:
    with pytest.raises(ValueError):
        actual_response = dog_api.create_vote(payload="Invalid")


def test_create_vote_wrong_payload_value_error(dog_api) -> None:
    with pytest.raises(ValueError):
        actual_response = dog_api.create_vote(payload={"image_id": "xyz"})
